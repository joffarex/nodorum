...
export class DeleteUserHandler implements ICommandHandler<DeleteUserCommand> {
  ...
  async execute(command: DeleteUserCommand): Promise<Result<string> | Result<void>> {
    const foundUser = await this._userRepository.findOne({ id: command.id });
    if (!foundUser) {
      return Result.fail(`User with id: ${command.id} not found`);
    }

    const user = this._publisher.mergeObjectContext(
      User.createWithId(foundUser.id, foundUser.username, foundUser.email, foundUser.password),
    );
    user.delete();
