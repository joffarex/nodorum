...
import { DownvoteCommentCommand } from './downvote-comment.command';
...
@CommandHandler(DownvoteCommentCommand)
export class DownvoteCommentHandler implements ICommandHandler<DownvoteCommentCommand> {
  constructor(
  ...
  async execute(command: DownvoteCommentCommand): Promise<Result<void>> {
    ...
    return await this._commentService.downvoteComment(comment, user, votes);
  }
}
