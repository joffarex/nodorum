...
import { UpvotePostCommand } from './upvote-post.command';
...
@CommandHandler(UpvotePostCommand)
export class UpvotePostHandler implements ICommandHandler<UpvotePostCommand> {
  constructor(
  ...
  async execute(command: UpvotePostCommand): Promise<Result<void>> {
    ...
    return await this._postService.upvotePost(post, user, votes);
  }
}
